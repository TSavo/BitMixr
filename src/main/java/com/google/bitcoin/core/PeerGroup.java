...
public class PeerGroup extends AbstractIdleService {
    private static final int DEFAULT_CONNECTIONS = 7;

    private static final Logger log = LoggerFactory.getLogger(PeerGroup.class);
    public final ReentrantLock lock = Locks.lock("peergroup");
    ...
    public void changePeer(){
    	handlePeerDeath(getDownloadPeer());
    }
    ...
    public ListenableFuture<Transaction> broadcastTransaction(final Transaction tx, final int minConnections) {
        ...
        peerAvailabilityFuture.addListener(new Runnable() {
			...
			public void run() {
                ...
                lock.lock();
                try {
                    somePeer = peers.get(new Random().nextInt(peers.size()));
                } finally {
                    lock.unlock();
                }
